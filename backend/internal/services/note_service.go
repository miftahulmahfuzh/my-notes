...
type NoteServiceInterface interface {
	...
	AddTagsToNote(userID, noteID string, tagNames []string) error
	RemoveTagsFromNote(userID, noteID string, tagNames []string) error
	GetNotesByTags(userID string, tagNames []string, operator string, limit, offset int) (*models.NoteList, error)
	GetNotesByAnyTag(userID string, tagNames []string, limit, offset int) (*models.NoteList, error)
	GetNotesByAllTags(userID string, tagNames []string, limit, offset int) (*models.NoteList, error)
	BatchAddTagsToNotes(userID string, noteIDs []string, tagNames []string) error
	BatchRemoveTagsFromNotes(userID string, noteIDs []string, tagNames []string) error
	GetTagUsageStats(userID string) (map[string]int, error)
}
...
// Enhanced Tag Operations for Day 3

// AddTagsToNote adds specific tags to a note
func (s *NoteService) AddTagsToNote(userID, noteID string, tagNames []string) error {
	ctx := context.Background()

	// Validate user owns the note
	_, err := s.GetNoteByID(userID, noteID)
	if err != nil {
		return fmt.Errorf("note not found or access denied: %w", err)
	}

	// Validate tag names
	if err := s.tagService.ValidateTagNames(tagNames); err != nil {
		return fmt.Errorf("invalid tag names: %w", err)
	}

	// Get existing tags for the note
	existingTags, err := s.getNoteTags(ctx, noteID)
	if err != nil {
	...
	// Process new tags
	if len(newTags) > 0 {
		if err := s.tagService.ProcessTagsForNote(noteID, newTags); err != nil {
...
func (s *NoteService) RemoveTagsFromNote(userID, noteID string, tagNames []string) error {
	ctx := context.Background()
	...
	// Get tag IDs for the tag names to remove
	for _, tagName := range tagNames {
		tag, err := s.tagService.GetTagByName(tagName)
		if err != nil {
			// Skip tags that don't exist
			continue
		}
		...
		if err != nil {
			return fmt.Errorf("failed to remove tag %s from note: %w", tagName, err)
		}
	}
...
// GetNotesByTags retrieves notes filtered by multiple tags with specified operator
func (s *NoteService) GetNotesByTags(userID string, tagNames []string, operator string, limit, offset int) (*models.NoteList, error) {
	ctx := context.Background()

	// Validate operator
	if operator != "and" && operator != "or" {
		return nil, fmt.Errorf("invalid operator: must be 'and' or 'or'")
	}
	...
	if operator == "and" {
		...
		query = `
			...
			AND EXISTS (
				...
				WHERE nt.note_id = n.id AND nt.tag_id = $2
			`
		args = []interface{}{userID, tagIDs[0]}

		for i := 1; i < len(tagIDs); i++ {
			query += fmt.Sprintf(`
				AND EXISTS (
					SELECT 1 FROM note_tags nt
					WHERE nt.note_id = n.id AND nt.tag_id = $%d
				)
			`, i+2)
		...
		query += `
			ORDER BY n.updated_at DESC
			LIMIT $` + strconv.Itoa(len(tagIDs)+2) + ` OFFSET $` + strconv.Itoa(len(tagIDs)+3)
		args = append(args, limit, offset)
	} else {
		...
		query = `
			SELECT DISTINCT n.id, n.user_id, n.title, n.content, n.created_at, n.updated_at, n.version
			FROM notes n
			INNER JOIN note_tags nt ON n.id = nt.note_id
			WHERE n.user_id = $1
			AND nt.tag_id IN (` + strings.Join(placeholders, ",") + `)
			ORDER BY n.updated_at DESC
			LIMIT $` + strconv.Itoa(len(tagIDs)+3) + ` OFFSET $` + strconv.Itoa(len(tagIDs)+4)
	...
	if operator == "and" {
		// For AND logic, count notes that have all specified tags
		countQuery = `
			SELECT COUNT(DISTINCT n.id)
			FROM notes n
			WHERE n.user_id = $1
			AND EXISTS (
				...
				WHERE nt.note_id = n.id AND nt.tag_id = $2
			`
		countArgs = []interface{}{userID, tagIDs[0]}
		...
		for i := 1; i < len(tagIDs); i++ {
			...
			`, i+2)
			countArgs = append(countArgs, tagIDs[i])
		}
		countQuery += ")"
	} else {
		// For OR logic, count notes that have any of the specified tags
		countQuery = `
			SELECT COUNT(DISTINCT n.id)
			FROM notes n
			INNER JOIN note_tags nt ON n.id = nt.note_id
	...
	rows, err := s.db.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("failed to get notes by tags: %w", err)
	}
	defer rows.Close()
...
// GetNotesByAnyTag retrieves notes that have any of the specified tags (OR logic)
func (s *NoteService) GetNotesByAnyTag(userID string, tagNames []string, limit, offset int) (*models.NoteList, error) {
	return s.GetNotesByTags(userID, tagNames, "or", limit, offset)
}

// GetNotesByAllTags retrieves notes that have all of the specified tags (AND logic)
func (s *NoteService) GetNotesByAllTags(userID string, tagNames []string, limit, offset int) (*models.NoteList, error) {
	return s.GetNotesByTags(userID, tagNames, "and", limit, offset)
}

// BatchAddTagsToNotes adds tags to multiple notes
func (s *NoteService) BatchAddTagsToNotes(userID string, noteIDs []string, tagNames []string) error {
	// Validate tag names
	if err := s.tagService.ValidateTagNames(tagNames); err != nil {
		return fmt.Errorf("invalid tag names: %w", err)
	}

	// Validate user owns all notes
	for _, noteID := range noteIDs {
		_, err := s.GetNoteByID(userID, noteID)
		if err != nil {
			return fmt.Errorf("note not found or access denied: %s", noteID)
		}
	}

	// Add tags to each note
	for _, noteID := range noteIDs {
		if err := s.AddTagsToNote(userID, noteID, tagNames); err != nil {
			return fmt.Errorf("failed to add tags to note %s: %w", noteID, err)
		}
	}
...
// BatchRemoveTagsFromNotes removes tags from multiple notes
func (s *NoteService) BatchRemoveTagsFromNotes(userID string, noteIDs []string, tagNames []string) error {
	// Validate user owns all notes
	for _, noteID := range noteIDs {
		_, err := s.GetNoteByID(userID, noteID)
		if err != nil {
	...
	// Remove tags from each note
	for _, noteID := range noteIDs {
		if err := s.RemoveTagsFromNote(userID, noteID, tagNames); err != nil {
			return fmt.Errorf("failed to remove tags from note %s: %w", noteID, err)
		}
	}
...
// GetTagUsageStats returns usage statistics for all tags for a user
func (s *NoteService) GetTagUsageStats(userID string) (map[string]int, error) {
	ctx := context.Background()
